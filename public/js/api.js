...
async function apiFetch(endpoint, options = {}) {
    ...
    try {
        ...
        // Si la respuesta no es JSON, lanzar error con el texto
        if (!contentType || !contentType.includes('application/json')) {
            const text = await res.text();
            // Si es HTML, mostrar un mensaje amigable
            if (text.includes('<!DOCTYPE') || text.includes('<html')) {
                throw new Error('El servidor devolvió HTML. Verifica que el controlador exista y la ruta sea correcta.');
            }
        ...
        // Si es un error de red, mostrar mensaje claro
...
async function loginUser(email, password, remember = false) {
    ...
    if (data.success && data.data.token) {
        ...
        if (remember) localStorage.setItem('bogaya_remember', 'true');
    }
    return data;
}
...
async function closeCaseApi(id) {
    return apiFetch('/cases/' + id + '/close', { method: 'PUT' });
}
...
// ========== TURNOS ==========
async function createAppointment(lawyer_id, fecha_hora, modalidad = 'virtual', case_id = null, notas_cliente = '', direccion = '') {
    return apiFetch('/appointments', {
        method: 'POST',
        body: JSON.stringify({ lawyer_id, fecha_hora, modalidad, case_id, notas_cliente, direccion })
    });
}
...
async function getReviewStats(lawyerId) {
    return apiFetch('/reviews/stats/' + lawyerId);
}
...
async function markNotificationRead(id = 0) {
...
async function verifyMatricula(userId, verified = 1) {
...
async function resetPasswordAdmin(userId, password) {
    return apiFetch('/admin/reset-password', {
        method: 'POST',
